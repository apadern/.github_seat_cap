...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    if (packageInfo) {
        ...
        if (subdirs.length > 1) {
            outdirParent = path.join.apply(null, subdirs);
        }
    ...
    // copy the template's static sap files to outdirParent (parent of webapp)
    var fromDir = path.join(templatePath, 'staticSAP');
    var staticSAPFiles = fs.ls(fromDir, 3);

    staticSAPFiles.forEach(function(fileName) {
        var toDir = fs.toDir( fileName.replace(fromDir, outdirParent) );
        fs.mkPath(toDir);
        copyFile(fileName, path.join(toDir, path.basename(fileName)), function(err){if(err) console.err(err);});
    });

    // copy user-specified static files to outdirParent
    var staticSAPFilePaths;
    var staticSAPFileFilter;
    var staticSAPFileScanner;
    if (conf.default.staticFiles) {
        // The canonical property name is `include`. We accept `paths` for backwards compatibility
        // with a bug in JSDoc 3.2.x.
        staticSAPFilePaths = conf.default.staticFiles.include ||
            conf.default.staticFiles.paths ||
            [];
        staticSAPFileFilter = new (require('jsdoc/src/filter')).Filter(conf.default.staticFiles);
        staticSAPFileScanner = new (require('jsdoc/src/scanner')).Scanner();

        staticSAPFilePaths.forEach(function(filePath) {
            var extraStaticFiles = staticSAPFileScanner.scan([filePath], 10, staticSAPFileFilter);
